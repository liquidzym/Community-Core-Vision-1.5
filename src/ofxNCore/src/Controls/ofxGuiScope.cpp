...
void ofxGuiScope::init(int id, string name, int x, int y, int width, int height, int length, ofVec2f value, int display)
{
...
void ofxGuiScope::setValue(ofVec2f value)
{
	//	todo:	steps
...
bool ofxGuiScope::update(int id, int task, void* data, int length)
{
	...
	if(id == mParamId)
	{
		if(task == kofxGui_Set_Float)
			setValue(ofVec2f(CLAMP(*(float*)data, 0.0, 1.0) * mBufferLength, mValue.y));
...
ofVec2f ofxGuiScope::valueToFraction(ofVec2f value)
{
	return ofVec2f(value.x / mBufferLength, value.y);
}

//	----------------------------------------------------------------------------------------------------

ofVec2f ofxGuiScope::fractionToValue(ofVec2f fraction)
{
	return ofVec2f(fraction.x * mBufferLength, fraction.y);
}
