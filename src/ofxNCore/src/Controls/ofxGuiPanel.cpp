...
bool ofxGuiPanel::mouseDragged(int x, int y, int button)
{
	...
	if(mMouseIsDown)
	{		
		ofVec2f inside = mouseToLocal(x, y);
...
ofxGuiObject* ofxGuiPanel::addXYPad(int id, string name, int width, int height, ofVec2f min, ofVec2f max, ofVec2f value, int display, int steps)
{
	int offset = (mObjects.size() == 0 && mParamName == "") ? 0 : mSpacing;
...
ofxGuiObject* ofxGuiPanel::addPoints(int id, string name, int width, int height, ofVec2f min, ofVec2f max, ofVec2f value, int display, int steps)
{
	int offset = (mObjects.size() == 0 && mParamName == "") ? 0 : mSpacing;
...
ofxGuiObject* ofxGuiPanel::addScope(int id, string name, int width, int height, int length, ofVec2f value, int display)
{
	int offset = (mObjects.size() == 0 && mParamName == "") ? 0 : mSpacing;
	...
	if(numberOfTags > 0)
	{
		for(int i = 0; i < numberOfTags; i++)
		{
			...
			else if(type == "XYPAD")
			{
				...
				ofVec2f min		= ofVec2f(minx, miny);
				ofVec2f max		= ofVec2f(maxx, maxy);
				ofVec2f value	= ofVec2f(valuex, valuey);
